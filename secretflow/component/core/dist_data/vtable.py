...
class VTable:
    ...
    def sort_partitions(self, order: list[str]) -> 'VTable':
        ...
        assert (
            set_order == set_keys
        ), f"unknown parties, {set_order.difference(set_keys)}"
    ...
    def from_distdata(
        ...
        if is_individual:
            assert (
                len(pb_schemas) == 1
            ), f"invalid individual schema size<{len(pb_schemas)}>"
        else:
            assert (
                len(pb_schemas) > 1
            ), f"invalid vertical schema size<{len(pb_schemas)}>"
...
class CompVDataFrame(IDumper):
    ...
    def dump(self, storage: Storage, uri: str, format: VTableFormat = VTableFormat.ORC) -> DistData:  # type: ignore
        ...
        assert lines[0] > 0, f"empty dataset is not allowed, line_count={lines[0]}"
        wait(closes)
...
class CompVDataFrameWriter:
    ...
    def dump(self) -> DistData:
        assert (
            self._schemas is not None and self.line_count > 0
...
def save_prediction(
    ...
    for f in addition_cols:
        assert (
            f not in addition_cols_set and f != pred_name
        ), f"do not select {f} as saved feature, repeated with id or label"
        assert (
            f in receiver_cols_set
    ...
    reader = CompVDataFrameReader(
        storage,
        tracer,
        tbl.select(reader_features),
    )
