...
from ..common.utils import dtype_to_type_str, to_attribute
...
class Reporter:
    ...
    def to_table(df: pd.DataFrame | pa.Table, name: str = "", desc: str = "", prefix: str = "") -> Table:  # type: ignore
        ...
        if isinstance(df, pd.DataFrame):
            for k in df.columns:
                dtype = dtype_to_type_str(df[k].dtype)
            ...
            for index, df_row in df.iterrows():
                rows.append(
                    Table.Row(
                        name=f'{prefix}{index}',
                        items=[to_attribute(df_row[k]) for k in df.columns],
                    )
                )
        elif isinstance(df, pa.Table):
            for f in df.schema:
                dtype = dtype_to_type_str(f.type)
    ...
    def to_descriptions(values: dict[str, int | float | bool | str], name: str = "", desc: str = "") -> Descriptions:  # type: ignore
        items = [
            Descriptions.Item(
                name=k, type=dtype_to_type_str(type(v)), value=to_attribute(v)
            )
