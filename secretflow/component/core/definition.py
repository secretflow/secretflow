...
class Field:
    ...
    def input(
        ...
        assert types, f"input types is none"
    ...
    def output(
        ...
        assert types, f"output types is none"
        types = [str(s) for s in types]
...
class Reflector:
    ...
    def _build_list_limit(is_list: bool, limit: Interval | None) -> tuple[int, int]:
        ...
        if limit.lower != None:
            assert limit.lower >= 0, f"list min size should be "
...
class Definition:
    def __init__(
        ...
        self._comp_def = ComponentDef(
            name=self.name,
            desc=_clean_text(desc, no_line_breaks=False),
            domain=self.domain,
    ...
    def make_checkpoint(self, param: NodeEvalParam | dict, checkpoint_uri: str) -> Checkpoint:  # type: ignore
        ...
        assert MINOR_NAME in kwargs, f"param must contain {MINOR_NAME}"
    ...
    def make_component(self, param: NodeEvalParam | dict, check_exist: bool = True) -> type[Component]:  # type: ignore
        ...
        assert MINOR_NAME in kwargs, f"param must contain {MINOR_NAME}, {kwargs}"
        minor = kwargs.pop(MINOR_NAME)
    ...
    def parse_param(self, param: NodeEvalParam) -> dict:  # type: ignore
    ...
    def _parse_attrs(self, param: NodeEvalParam) -> dict:  # type: ignore
        ...
        for path, attr in zip(list(param.attr_paths), list(param.attrs)):
            ...
            if path not in self._attr_types:
                raise ValueError(f"unknown attr key {path}")
