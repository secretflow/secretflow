...
class ModelExport(Component):
    ...
    def rebuild_preprocessing_nodes(
        ...
        assert len(builder.nodes) > 0, "empty nodes"
    ...
    def rebuild_schema(
        ...
        for f in trace_schema:
            ...
            if is_input:
                new_trace_fields.append(in_schema.field(name))
            else:
                new_trace_fields.append(f)
                new_in_fields.append(f)

            if is_output:
                out_f = out_schema.field(name)
                assert (
                    out_f.type == f.type
                ), f"type mismatch, {f.type} != {out_f.type}, trace_schema={trace_schema}, in_schema={in_schema}, out_schema={out_schema}"
            else:
                new_out_fields.append(f)

        ext_trace_fields = [f for f in in_schema if f.name not in trace_schema.names]
        trace_schema = pa.schema(new_trace_fields + ext_trace_fields)
