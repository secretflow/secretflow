...
class WriteData(Component):
    ...
    def evaluate(self, ctx: Context):
        if self.write_data_type == str(DistDataType.BINNING_RULE):
            ...
            rule_objs = bin_rule_from_pb_and_old_rule(
                model.objs, model.public_info, bin_rules
            )

            info_dict = model.public_info
            info_dict["model_hash"] = uuid4(model.objs[0].device)
            out_model = Model(
                name="bin_rule",
                type=DistDataType.BINNING_RULE,
                version=BINNING_RULE_MAX,
                objs=rule_objs,
                public_info=info_dict,
