...
class SecurePruneAggregator(Aggregator):
    ...
    Warnings:
        The SecurePruneAggregator uses :py:meth:`numpy.random.PCG64`. There are many
    ...
    Examples:
        ...
        dtype=float32)
        >>> average_a_b = aggregator.average([a, b],[mask_a, mask_b] axis=0)
    ...
    def average(
        ...
        # mask data: data= weights
        self._check_data(data)
        masked_data = [None] * len(data)  # init none
        dtypes = [None] * len(data)
        _weights = []
