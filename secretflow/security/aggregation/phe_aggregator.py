...
class _WorkerOperator:
    ...
    def __init__(self, device, public_key, paillier_key_size=2048, len_after_encode=55, bias_of_weight=5, expand_of_weight=10e12):
        self._device = device
        logging.info(f"WorkerOperator36{device}{public_key.device}")
...
class _AggregatorOperator:
    ...
    def __init__(self, device, amount_of_party, paillier_key_size=2048, len_after_encode=55, bias_of_weight=5, expand_of_weight=10e12):
        self._device = device
        ...
        for i in range(1, len(local_weights_after_enc_list)):
            for m in range(len(local_weights_after_enc_list[i])):
                for n in range(len(local_weights_after_enc_list[i][m])):
                    self._evaluator.add_inplace(enc_add_list[m][n], local_weights_after_enc_list[i][m][n])
        time_end_dec = time.time()
        # logging.info(f"The homomorphic aggregation takes{time_end_dec - time_start_dec}")
        return enc_add_list

    def homo_dec_list(self, enc_add_list, local_weight_shape, size_of_per_col):  # 解密整个加密聚合后的列表
        ...
        for index in range(len(enc_add_list)):
            ...
            for i in range(len(enc_add_list[index])):
                dec_list[index].append(re.findall(r'\w{%d}' % self._len_after_encode,
            ...
            dec_list[index] = torch.tensor([(int(i, 2) / self._expand_of_weight - self._amount_of_party * self._bias_of_weight) for i in
...
class PheAggregator(Aggregator):
    ...
    def __init__(self, device: PYU, participants: List[PYU], paillier_key_size=2048, len_after_encode=55, bias_of_weight=5, expand_of_weight=10e12):
        ...
        self._aggregator_operator = _AggregatorOperator(self.device, self._amount_of_party, device=self.device)

        self._public_key = self._aggregator_operator.get_public_key()

        # self._public_key_todevice = {
        #     pyu: self._public_key.to(pyu) for pyu in participants
        # }

        self._worker_operator = {
            pyu: _WorkerOperator(pyu, public_key= self._public_key,
                                 paillier_key_size=self._PAILLIER_KEY_SIZE, len_after_encode=55,
                                 bias_of_weight=5, expand_of_weight=10e12, device=pyu) for pyu in participants
        }

        # for device,worker in self._worker_operator.items():
        #     worker.phe_setup()
    ...
    def average(self, data: List[DeviceObject], axis=None, weights=None) -> PYUObject:
        ...
        self._local_weight_shape = self._worker_operator[data_operate[0].device].get_shape(data_operate[0]).to(self.device)
        self._size_of_per_col = self._worker_operator[data_operate[0].device].get_size_of_per_col(data_operate[0]).to(self.device)

        data_operate = [self._worker_operator[d.device].paillier_enc_local_weights(d).to(self.device) for d in data_operate]

        added_weight_list = self._aggregator_operator.homo_add(data_operate)

        dec_list = self._aggregator_operator.homo_dec_list(added_weight_list, self._local_weight_shape, self._size_of_per_col)
        return dec_list