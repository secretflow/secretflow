...
class _Enc:
	def __init__(self, party, TA, fxp_bits: int = 18):
		self.party = party
		self.pub_key, self.pri_key, self.security_parameter = TA.genUserParameters(party)
...
class TA:
	def __init__(self, device: PYU, participants: List[PYU], security_parameter: int = 1024, threshold: int = 1):
		self.security_parameter = security_parameter
		self.participants = set(participants)
		self.threshold = threshold
		self.public_key, self.private_key = paillier.generate_paillier_keypair(n_length=self.security_parameter)
		self.ids = {str(pyu.party): random.SystemRandom().randrange(self.private_key.p) for pyu in self.participants}
		...
		coff = [random.SystemRandom().randrange(self.private_key.p) for i in range(self.threshold-1)]
		self.sk = {}
		for party in self.ids.keys():
			tmp = [self.ids[str(party)]**i for i in range(1,self.threshold)]
			fx = np.dot(tmp,coff) % self.private_key.p
			self.sk[str(party)] = user_sk(powmod(s, fx*self.private_key.q,self.public_key.nsquare), self.h)
...
class SecureAggregator(Aggregator):
	def __init__(self, device: PYU, TA, participants: List[PYU], fxp_bits: int = 18):
		...
		self.encs = {
		    pyu: _Enc(pyu.party, TA, fxp_bits = self.fxp_bits, device=pyu) for pyu in participants
		}
			...
			for i in range(row):
				for j in range(column):
					for z in range(len(enc_data)):
						results[i][j] = mulmod(results[i][j], enc_data[z][i][j], self.public_key.nsquare)
					results[i][j] = self.private_key.raw_decrypt(results[i][j]) % self.p1
