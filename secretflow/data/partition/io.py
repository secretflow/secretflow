...
def read_csv_wrapper(
    filepath: str, auto_gen_header_prefix: str = "", backend="pandas", **kwargs
    ...
    def _read_csv(_filepath, _backend, *_args, **_kwargs):
        if _backend == "pandas":
            ...
            from secretflow.data.partition.polars.util import read_polars_csv
    ...
    if auto_gen_header_prefix:
        kwargs['header'] = None
        df = _read_csv(filepath, backend, **kwargs)
        df.columns = [
