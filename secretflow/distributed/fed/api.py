...
def _shutdown(intended, on_error):
    ...
    if on_error and not get_global_context().get_local_exception_raised():
        wait_for_sending = False

    if (
        get_global_context().get_remote_exception_recved()
        or get_global_context().get_send_exception_raised()
    ):
        wait_for_sending = False
    ...
    clear_global_context(wait_for_sending)
...
def get(objects: Any):
    ...
    try:
        ...
        get_global_context().set_remote_exception_recved()
        logger.warning(
        ...
        get_global_context().set_local_exception_raised()
        raise
