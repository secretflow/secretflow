...
class GlobalContext:
    def __init__(
        ...
        self._remote_exception_recved = False
        self._local_exception_raised = False
        self._send_exception_raised = False
        self._atomic_shutdown_flag = True
        self._clean_queue = Queue()
        self._clean_stoped = False
    ...
    def set_remote_exception_recved(self):
        with self._lock:
            self._remote_exception_recved = True

    def get_remote_exception_recved(self):
        with self._lock:
            return self._remote_exception_recved

    def set_local_exception_raised(self):
        with self._lock:
            self._local_exception_raised = True

    def get_local_exception_raised(self):
        with self._lock:
            return self._local_exception_raised

    def set_send_exception_raised(self):
        with self._lock:
            self._send_exception_raised = True

    def get_send_exception_raised(self):
        with self._lock:
            return self._send_exception_raised
    ...
    def _cleanup(self):
        while True:
            ...
            if not self._clean_stoped and callable(task):
                if not task():
                    break
            else:
                break
    ...
    def stop(self, wait_for_sending=True):
        main_thread_assert()
        logger.info(f"Try stop context, wait_for_sending {wait_for_sending}")
        self._task_executor.shutdown(cancel_futures=not wait_for_sending)
        logger.info("task_executor stoped")
        self._recv_executor.shutdown(cancel_futures=not wait_for_sending)
        logger.info("recv_executor stoped")
        self._send_executor.shutdown(cancel_futures=not wait_for_sending)
        logger.info("send_executor stoped")

        if not wait_for_sending:
            self._clean_stoped = True
        self._clean_queue.put("STOP")
        self._clean_thread.join()
        logger.info("clean_thread stoped")
        self._proxy.stop()
        logger.info("proxy stoped")
...
def clear_global_context(wait_for_sending=False):
    ...
    if _global_context is not None:
        _global_context.stop(wait_for_sending)
        _global_context = None
        gc.collect()
