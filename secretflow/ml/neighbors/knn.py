...
def _fit(x: jnp.ndarray, y: jnp.ndarray, model) -> neighbors.KNNClassifer:
    model.fit(x, y)
    return model


class KNNClassifer(_ModelBase):
    ...
    def predict(self, x: Union[FedNdarray, VDataFrame]) -> SPUObject:
        ...
        res = self.spu(_predict)(spu_x, self.model)
        return res
