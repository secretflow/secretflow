...
def tensorDCT(weight_tensor):
    ...
    # 对每一行进行DCT
    dct_rows = torch.tensor([dct(row, type=2, norm='ortho') for row in weight_tensor_cpu])
    # 对每一列进行DCT
...
def clustering(F_list):
    ...
    for i in range(K):
        for j in range(K):
            # 1 - 余弦相似度
            distances_matrix[i, j] = 1 - torch.nn.functional.cosine_similarity(F_list[i].unsqueeze(0), 
                                                                               F_list[j].unsqueeze(0))
            # 保证对称性
    ...
    clusterer = HDBSCAN(metric="precomputed", min_cluster_size=2, min_samples=1) #至少有一个邻居
