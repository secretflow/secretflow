...
# The code is modified from sl_model.py, and Split Guard detection from the paper <SplitGuard: Detecting and Mitigating Training-Hijacking Attacks in Split Learning>  is added.
...
def sg_score(fakes, R1, R2, shift=0, mult=1, exp=1, raw=False):
    ...
    R_mean = (R1_mean + R2_mean) / 2  # mean of R
    ...
    R_mean_mag = (R1_mean_mag + R2_mean_mag) / 2  # mean of ||R||
    
    # d(F,R)+d(R_1,R_2) 
    mag_div = abs(F_mean_mag - R_mean_mag) + abs(
        R1_mean_mag - R2_mean_mag
    )  

    # compute S as in equation (6).
    ...
    if raw:
        ...
        return sigmoid(S, shift=shift, mult=mult, exp=exp)
...
class SLModel_SG:
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, steps_per_epoch + self.pipeline_size - 1):
                ...
                if step < (steps_per_epoch + self.pipeline_size - 2):
                    ...
                    client_grad = sf.reveal(grad_client_np)[0].flatten()
                    # Fake batches
                    if step in [11, 21, 31, 41, 51, 61, 71]:
                        ...
                        if (
                            ...
                            sg = sg_score(
                                self.fake, self.r_1, self.r_2, mult=5, exp=2, raw=False
                            )
    ...
    def plot_sgscore(self):
        ...
        plt.plot(self.results, label="attack")
        plt.ylim(0, 1.1)
