...
############add split guard here
                    ...
                    # print("6")
                    # print("sf.reveal(grad_client_np)",sf.reveal(grad_client_np))
                    client_grad =sf.reveal(grad_client_np)[0].flatten()
                    # print(client_grad.shape)
                    # print("sf.reveal(grad_client_np)[0].clone()",client_grad)
                    if step in [11,21,31,41,51,61,71]:
                        self.fake.append(client_grad)
                        if len(self.r_1) > 0 and len(self.r_2)>0 and len(self.fake)> 0:
                            sg = sg_score(self.fake, self.r_1, self.r_2, mult=5, exp=2, raw=False)
