...
class SLBaseModel(ABC):
    def __init__(
        self,
        builder_base: Callable,
        builder_fuse: Callable = None,
        builder_decoder: Callable = None,
    ):
        ...
        self.model_decoder = builder_decoder() if builder_decoder is not None else None
...
class SLBaseTFModel(SLBaseModel):
    def __init__(
        ...
        builder_decoder: Callable[[], tf.keras.Model],
        ...
        **kwargs,
        ...
        super().__init__(builder_base, builder_fuse, builder_decoder)
    ...
    def _base_backward(self, gradient, compress: bool = False):
        """backward on fusenet
    ...
    def unsplit_base_backward(self, gradient, compress: bool = False):
        """backward on fusenet
    ...
    def server_net(self, *hidden_features, _num_returns=2, compress=False, vctim_num=0):
        """Fuses the hidden layer and calculates the reverse gradient
        only on the side with the label
        ...
        logs = {}
        gradients = self._server_net_train(hiddens, vctim_num)
        for m in self.model_fuse.metrics:
            logs["train_" + m.name] = m.result().numpy()
        self.logs = logs
        if compress:
            ...
            if fuse_sparse_masks:
                assert len(fuse_sparse_masks) == len(
                    gradients
                ), f"length of fuse_sparse_masks and gradient mismatch: {len(fuse_sparse_masks)} - {len(loss)}"
    ...
    def unsplit_net(
        self, *hidden_features, _num_returns=2, vctim_num=0, compress=False
    ):
        """Fuses the hidden layer and calculates the reverse gradient
        only on the side with the label
    ...
    def _server_net_train(self, hiddens, vctim_num):
        gradients = []
        vctim_flag = False
    ...
    def _unsplit_net_train(self, hiddens, vctim_num):
        gradients = []
        for i, h in enumerate(hiddens):
            if i == vctim_num:
                gradients.append(
                    self._unsplit_net_internal(
                        h,
                        self.train_sample_weight,
                        self.train_y,
                    )
                ...
                gradients.append(
                    self._unsplit_net_internal_fake(
                        h,
                        self.train_sample_weight,
                        self.train_y,
                    )
    ...
    def _server_net_internal(
        self, hiddens, train_sample_weight, train_label, vctim_flag
    ):
        # trainable_vars_base = self.model_base.trainable_variables
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(hiddens)
            # Step 1: forward pass
            y_pred = self.model_fuse(hiddens, training=True, **self.kwargs)
            ...
            losses = tf.reduce_mean(
                tf.keras.losses.categorical_crossentropy(train_label, y_pred)
            )
            if vctim_flag and self.model_decoder != None:
                rec_data = self.model_decoder(hiddens, training=True)
                l = tf.losses.MeanSquaredError()
                decoder_loss = l(self.data_x, rec_data)
        # Step3: compute gradients
        trainable_vars = self.model_fuse.trainable_variables
        gradients = tape.gradient(losses, trainable_vars)
        if vctim_flag:
            if self.model_decoder != None:
                decoder_vars = self.model_decoder.trainable_variables
                decoder_gradients = tape.gradient(decoder_loss, decoder_vars)
                self.model_decoder.optimizer.apply_gradients(
                    zip(decoder_gradients, decoder_vars)
                )

            self.model_fuse.optimizer.apply_gradients(zip(gradients, trainable_vars))
            # Step4: update metrics
            self.model_fuse.compiled_metrics.update_state(
                train_label, y_pred, sample_weight=train_sample_weight
            )

        return tape.gradient(losses, hiddens)

    def _unsplit_net_internal(self, hiddens, train_sample_weight, train_label):
        # trainable_vars_base = self.model_base.trainable_variables
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(hiddens)
            # Step 1: forward pass
    ...
    def _unsplit_net_internal_fake(self, hiddens, train_sample_weight, train_label):
        # trainable_vars_base = self.model_base.trainable_variables
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(hiddens)
            # Step 1: forward pass
    ...
    def evaluate_(self, *hidden_features, vctim_num: int = 0, compress: bool = False):
        """Returns the loss value & metrics values for the model in test mode.
    ...
    def evaluate_unsplit(self, *hidden_features, compress: bool = False):
        """Returns the loss value & metrics values for the model in test mode.
    ...
    def attack(self, *hidden_features, compress: bool = False):
        """Generates output predictions for the input hidden layer features.
    ...
    def save_decoder_model(self, decoder_model_path: str, **kwargs):
        Path(decoder_model_path).parent.mkdir(parents=True, exist_ok=True)
        assert decoder_model_path is not None, "model path cannot be empty"
        self.model_decoder.save(decoder_model_path, **kwargs)
    ...
    def load_decoder_model(self, decoder_model_path: str, **kwargs):
        self.init_data()
        assert decoder_model_path is not None, "model path cannot be empty"
        self.model_decoder = tf.keras.models.load_model(decoder_model_path, **kwargs)
    ...
    def export_decoder_model(
        self, model_path: str, save_format: str = "onnx", **kwargs
    ):
        return self._export_model(self.model_decoder, model_path, save_format, **kwargs)
