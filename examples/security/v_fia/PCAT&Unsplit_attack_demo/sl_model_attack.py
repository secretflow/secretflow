...
from sl_base_attack import PYUSLTFModel
...
class SLModel_attack:
        ...
        if strategy == "split_nn":
            self._workers = {
                device: PYUSLTFModel(
                    ...
                    builder_decoder=None if device != device_y else model_decoder,
    ...
    def fit_pcat(
        ...
        fuse_net_num_returns = sum(self.basenet_output_num.values())
        # fuse_net_num_returns = list(self.basenet_output_num.values())[0]
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, steps_per_epoch):
                ...
                gradients = self._workers[self.device_y].server_net(
                    *hiddens,
                    _num_returns=fuse_net_num_returns,
                    compress=self.has_compressor,
                    vctim_num=vctim_num,
                ...
                for device, worker in self._workers.items():
                    ...
                    for i in range(self.basenet_output_num[device]):
                        gradient = gradients[idx + i].to(device)
                        gradient_list.append(gradient)
                    worker._base_backward(gradient_list, compress=self.has_compressor)
                    idx += self.basenet_output_num[device]
                r_count = self._workers[self.device_y].on_train_batch_end(step=step)
                res.append(r_count)
                if self.dp_strategy_dict is not None and dp_spent_step_freq is not None:
            ...
            if validation and epoch % validation_freq == 0:
                ...
                for step in range(0, valid_steps):
                    ...
                    for device, worker in self._workers.items():
                        hidden = worker.base_forward("eval")
                        hiddens.append(hidden.to(self.device_y))
                    metrics = self._workers[self.device_y].evaluate_(
                        *hiddens, vctim_num
                    )
                    res.append(metrics)
                    if len(res) == wait_steps:
    ...
    def fit_unsplit(
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, steps_per_epoch):
                ...
                gradients = self._workers[self.device_y].unsplit_net(
                    *hiddens,
                    _num_returns=fuse_net_num_returns,
                    compress=self.has_compressor,
                    vctim_num=vctim_num,
                ...
                for step in range(0, valid_steps):
                    ...
                    for device, worker in self._workers.items():
                        if device != self.device_y:
                            hidden = worker.base_forward("eval")
                            hiddens.append(hidden.to(self.device_y))
                    metrics = self._workers[self.device_y].evaluate_unsplit(*hiddens)
                    res.append(metrics)
                    if len(res) == wait_steps:
                        wait(res)
                        res = []
    ...
    def predict(
        ...
        for step in range(0, predict_steps):
            ...
            for device, worker in self._workers.items():
                ...
                # print(sf.reveal(hidden))
                hiddens.append(hidden.to(self.device_y))
            if verbose > 0:
                pbar.update(1)
            # y_pred = self._workers[self.device_y].predict(*hiddens, compress=compress)
            y_pred = self._workers[self.device_y].attack(*hiddens, compress=compress)
    ...
    def save_model(
        ...
        **kwargs,
        ...
        if with_training_decoder:
            assert decoder_model_path is not None, "decoder model path cannot be empty"
        ...
        if with_training_decoder:
            res.append(
                self._workers[self.device_y].save_decoder_model(
                    decoder_model_path, **kwargs
                )
            )
        wait(res)
