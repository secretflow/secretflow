...
from ndarray_attack import create_ndarray, create_ndarray_copy
...
def load_mnist_pcat_attack(
    ...
    Returns:
        A tuple consists of two tuples, (x_train, y_train) and (x_train, y_train).
    """
    filepath = _get_dataset(_DATASETS["mnist_"])
    with np.load(filepath) as f:
        x_train, y_train = f["x_train"], f["y_train"]
        x_test, y_test = f["x_test"], f["y_test"]
    ...
    if reshape_:
        ...
        x_train = numpy.array(parse(x_train))
        x_test = numpy.array(parse(x_test))
    ...
    return (
        (
            ...
            create_ndarray_copy(y_train, parts=parts, axis=0, is_labal=True),
        ),
        (
            create_ndarray_copy(
                x_test,
            ...
            create_ndarray_copy(y_test, parts=parts, axis=0, is_labal=True),
        ),
    )
