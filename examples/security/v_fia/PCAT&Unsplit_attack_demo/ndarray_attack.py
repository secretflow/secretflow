...
import numpy as np
from _utils_attack import cal_indexes, restrict_copy_samesize_data
...
def create_ndarray_copy(
    source: Union[str, np.ndarray, Callable],
    parts: Union[List[PYU], Dict[PYU, Union[float, tuple]]],
    label: Union[str, np.ndarray, Callable] = None,
    axis: int = 0,
    is_labal=False,
    shuffle: bool = False,
    random_state: int = None,
    ...
    if isinstance(source, str):
        ...
        if is_labal:
            ...
            arr_label = np.load(label, allow_pickle=allow_pickle)
    elif isinstance(source, np.ndarray):
        arr = source
        if is_labal:
            pass
    ...
    if is_torch:
        ...
        if is_labal:
            ...
            arr_label = arr_label[:, np.newaxis, :, :]
    if shuffle:
        arr = np.random.default_rng(random_state).shuffle(arr)
        if is_labal:
            pass
        else:
            arr_label = np.random.default_rng(random_state).shuffle(arr_label)
    ...
    parts_num = list(parts.values())
    vctim_num_ = 0

    vctim_num = parts_num[0]
    attack_num = parts_num[1]
    if attack_num > vctim_num:
        attack_num = parts_num[0]
        vctim_num = parts_num[1]
        vctim_num_ = 1
    ...
    if is_labal:
        ...
        arr_server = restrict_copy_samesize_data(arr, arr_label, vctim_num, attack_num)
    ...
    for device in devices:
        ...
        device_.append(device)

    if axis == 0:
        if is_labal:
            partion = {
                device: device(lambda df: df[0:vctim_num])(arr) for device in devices
            }
        else:
            if vctim_num_ == 0:
                partion = {
                    device_[0]: device_[0](lambda df: df[0:vctim_num])(arr),
                    device_[1]: device_[1](lambda df: df[0:vctim_num])(arr_server),
                }
            else:
                partion = {
                    device_[0]: device_[0](lambda df: df[0:vctim_num])(arr_server),
                    device_[1]: device_[1](lambda df: df[0:vctim_num])(arr),
                }
