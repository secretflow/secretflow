...
def restrict_copy_samesize_data(
    arry: ndarray, label: ndarray, vctim_num: int, attacker_num: int
) -> ndarray:
    """Calculate the indexes by the given partitions.
    ...
    array = arry.tolist()[:attacker_num]
    server_arr = []
    num = 0
    while num < vctim_num:
        for i in range(attacker_num):
            if label_list[num] == label_list[i]:
                server_arr.append(array[i])
                num += 1
                if num == vctim_num:
                    break

    return np.array(server_arr)
