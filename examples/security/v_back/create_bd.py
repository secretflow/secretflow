...
def createbd(x,gradient_step,cicle,model):
    ...
    for i in range(cicle):
        ...
        with torch.no_grad():
            ...
            tensor[:,:,23:30,23:30] = (tensor[:,:,23:30,23:30] - gradient_step * tensor.grad[:,:,23:30,23:30])
    ...
    for i in range(350):
        ...
        with torch.no_grad():
                # apply gradient descent
            tensor[:, :, 23:30, 23:30] = (tensor[:, :, 23:30, 23:30] - 0.001 * tensor.grad[:, :, 23:30, 23:30])
    ""
    for i in range(300):
        tensor.requires_grad = True
        ...
        with torch.no_grad():
                # apply gradient descent
            tensor[:, :, 23:30, 23:30] = (tensor[:, :, 23:30, 23:30] - 0.0001 * tensor.grad[:, :, 23:30, 23:30])
    ""
    for i in range(300):
        tensor.requires_grad = True
        ...
        with torch.no_grad():
                # apply gradient descent
            tensor[:, :, 23:30, 23:30] = (tensor[:, :, 23:30, 23:30] - 0.00005 * tensor.grad[:, :, 23:30, 23:30])
    ""
