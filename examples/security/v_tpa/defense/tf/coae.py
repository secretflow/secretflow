...
if gpus:
    try:
        ...
        tf.config.experimental.set_virtual_device_configuration(
            gpus[1], [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=10240)])
...
if __name__ == '__main__':
    ...
    for num_classes in [5, 10]:
        for ew in [0.01, 0.03, 0.05, 0.07, 0.10]:
            ...
            y_onehot = F.one_hot(indices=np.random.randint(num_classes, size=(test_size,)), 
            ...
            dataset = tf.data.Dataset.from_tensor_slices(y_train).shuffle(buffer_size=train_size).batch(batch_size)
            for i in range(epochs):
                ...
                if (i + 1) % 10 == 0:
                    ...
                    acc_p = len(tf.where((F.argmax(y_hat, axis=1) == y_labels) == True)) / len(y_labels)
                    acc_n = len(tf.where((F.argmax(y_latent, axis=1) != y_labels) == True)) / len(y_labels)
                    acc_p_onehot = len(tf.where((F.argmax(y_hat_onehot, axis=1) == y_labels_onehot) == True)) / len(y_labels_onehot)
                    acc_n_onehot = len(tf.where((F.argmax(y_latent_onehot, axis=1) != y_labels_onehot) == True)) / len(y_labels_onehot)
                    cur_loss = total_loss / batch_count
                    print('epoch', i + 1, 'acc_p:', acc_p, 'acc_n:', acc_n, 'acc_p_onehot:', acc_p_onehot, 'acc_n_onehot:', acc_n_onehot, 'loss:', cur_loss.numpy())
