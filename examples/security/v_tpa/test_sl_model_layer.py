...
from test_model.tf.cifar10_model import Splittable_Model, split_options
from test_model.tf.basic_sl_model import create_passive_model, create_fuse_model

config_logger(fname='./logs/baseline_layer.log')
party_num = PARTY_NUM

for split_point in split_options:
    sf.shutdown()
            ...
            for blurred in [True]:
                for method in METHODS:
                    ...
                    for i in range(TIMES):
                        splittable_model = Splittable_Model(len(labels_set))
                        bottom_model, fuse_model = splittable_model.split(split_point, party_num)
                        ...
                        dst_dataset = ds_config['badnets'](ds_name, ds_path, ds_args, BADNETS_ARGS)
                        POISONING_ARGS['train_poisoning_indexes'] = dst_dataset.get_train_poisoning_indexes()
                        POISONING_ARGS['valid_poisoning_indexes'] = dst_dataset.get_valid_poisoning_indexes()
                        POISONING_ARGS['train_target_indexes'] = dst_dataset.get_train_target_indexes()
                        POISONING_ARGS['valid_target_indexes'] = dst_dataset.get_valid_target_indexes()
                        
                        train_passive_datas, train_active_data = dst_dataset.split_train(party_num=party_num, channel_first=False)
                        valid_passive_datas, valid_active_data = dst_dataset.split_valid(party_num=party_num, channel_first=False)
                        ...
                        data_files = {
                            'data/y_train.npy': F.one_hot(train_active_data.party_labels, num_classes).float().numpy(),
                            'data/y_valid.npy': F.one_hot(valid_active_data.party_labels, num_classes).float().numpy(),
                        }
                        ...
                        bottom_models = {}
                        intermediate_shapes = []
                        for feature_shape, party_device in zip(party_feature_shapes, party_devices):
                            bottom_models[party_device] = create_passive_model(
                                    feature_shape, len(labels_set), Splittable_Model,
                                    split_point, party_num,
                                    opt_args, bottom_compile_args)

                            # query the intermediate shape
                            with tf.device('/cpu:0'):
                                dummy_inputs = tf.zeros((10,) + feature_shape)
                                dummy_model = bottom_models[party_device]()
                                dummy_intermediate = dummy_model(dummy_inputs)
                                intermediate_shapes.append(dummy_intermediate.shape[1:])

                        model_fuse = create_fuse_model(intermediate_shapes, agg, len(labels_set),
                                Splittable_Model, split_point, party_num, opt_args, fuse_compile_args)
                        ...
                        y_train = load({party_devices[0]: data_names['y_train']}, allow_pickle=True)
                        y_valid = load({party_devices[0]: data_names['y_valid']}, allow_pickle=True)
                        ...
                        history = sl_model.fit(x=x_train,
                                     y=y_train,
                        ...
                        asr_result = asr(preds_plain.numpy(), y_labels, target_class,
                                         np.arange(len(y_labels)),
                                         POISONING_ARGS["valid_poisoning_indexes"])
    
                        print_log(['{"dataset_name":"' + ds_name + '",',
                                     '"method":"' + method + '",',
                                     '"split_point":{}'.format(split_point) + ',',
