...
def weights_init_normal(m):
    if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d):
        init.normal_(m.weight, mean=1.0, std=1e-1)
        init.normal_(m.bias, mean=1.0, std=1e-1)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, kernel_size, stride=1, option="A"):
        ...
        self.conv1 = Conv2dSame(
            in_planes, planes, kernel_size=kernel_size, stride=stride, bias=False
        )
        self.bn1 = nn.BatchNorm2d(planes)
        # self.conv2 = nn.Conv2d(planes, planes, kernel_size=kernel_size, stride=1, padding=1, bias=False)
        self.conv2 = Conv2dSame(
            planes, planes, kernel_size=kernel_size, stride=1, bias=False
        )
