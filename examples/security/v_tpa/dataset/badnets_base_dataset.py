...
class BadNetsBaseDataset:
    def __init__(self, train_dataset, valid_dataset, badnets_args):
        ...
        if self.train_poison_ratio > 0.0:
            self.train_poisoning_indexes = np.random.choice(range(len(self.train_dataset)),
        ...
        if self.valid_poison_ratio > 0.0:
            self.valid_poisoning_indexes = np.random.choice(range(len(self.valid_dataset)),
                                             int(self.valid_poison_ratio * len(self.valid_dataset)),
                                             replace=False)
        else:
            self.valid_poisoning_indexes = []
        self.train_target_indexes = self.build_target_set(train_dataset, self.train_known_target_num)
        self.valid_target_indexes = self.build_target_set(valid_dataset, self.valid_known_target_num)
