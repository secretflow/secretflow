            ...
            for blurred in [True]:
                for method in METHODS:
                    ...
                    for i in range(TIMES):
                        ...
                        coae_y_train = (
                            F.one_hot(train_active_data.party_labels, num_classes)
                            .float()
                            .numpy()
                        )
                        coae_y_train = coae.encoder(tf.convert_to_tensor(coae_y_train))
                        coae_y_valid = (
                            F.one_hot(valid_active_data.party_labels, num_classes)
                            .float()
                            .numpy()
                        )
                        ...
                        for pred in preds:
                            preds_plain.append(
                                tf.argmax(coae.decoder(reveal(pred)), axis=1)
                            )
                        preds_plain = tf.concat(preds_plain, axis=0)
                        y_labels = valid_active_data.party_labels
                        acc = len(tf.where(preds_plain == y_labels) == True) / len(
                            y_labels
                        )
                        asr_result = asr(
                            preds_plain.numpy(),
