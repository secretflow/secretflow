...
class ResidualBlock(keras.Model):
    def __init__(self, out_channels, kernel_size, padding, strides):
        ...
        self.conv_res1 = layers.Conv2D(out_channels, kernel_size=kernel_size,
                                   padding=padding, strides=strides, use_bias=False)
        self.conv_res1_bn = layers.BatchNormalization(momentum=0.9)
        self.conv_res2 = layers.Conv2D(out_channels, kernel_size=kernel_size,
                                   padding=padding, use_bias=False)
        self.conv_res2_bn = layers.BatchNormalization(momentum=0.9)

        if strides != 1:
            self.downsample = nn.Sequential(
                layers.Conv2D(out_channels, kernel_size=1, strides=strides, use_bias=False),
