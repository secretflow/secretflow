...
import numpy as np
import copy
from examples.security.h_bd_cla.backdoor_BCL_torch import poison_dataset, test_img
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, train_steps_per_epoch, aggregate_freq):
                ...
                for idx, device in enumerate(self._workers.keys()):
                    ...
                    callbacks.on_train_batch_inner_middle(epoch, device)
                    if device == attack_party:
                          if epoch >= attack_epoch:
                                client_params_poison, sample_num_poison = self._workers[device].train_step(
                                    client_params,
                                    epoch * train_steps_per_epoch + step,
                                    (
                                        aggregate_freq
                    ...
                    callbacks.on_train_batch_inner_after(epoch, device)

                    # As 'callback' has no return value, we modify the params here in this way. 
                    if device == attack_party:
                        if epoch >= attack_epoch:
                            # gamma = len(self._workers) / attack_eta #attack

                            def BLS_weight(
                                key_arr, value_arr, model_benign, model_malicious, BSR, mal_val_dataset, args, global_model, threshold=0.8
                            ):
                            ...
                            def atapdtive_attack(
                                attack_worker, malicious_info, global_model, args
                            ):
                                ...
                                crafted_model = craft_model(
                                    ...
                                    lambda_val)
                                attack_worker.set_weights(crafted_model)
                                return crafted_model
                            ...
                            client_params = self._workers[attack_party].apply(
                                 atapdtive_attack, malicious_info, self._workers[attack_party].global_model, args
                            )
