...
from diffusers.models.attention_processor import CustomDiffusionAttnProcessor,CustomDiffusionXFormersAttnProcessor, Attention
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        help="A folder containing the training data of adversarial images.",
    )
    parser.add_argument(
        "--class_data_dir",
        type=str,
...
def train_dreambooth(
    ...
    for step in range(num_steps):
        ...
        if True:
            ...
            batch = collate_fn([step_data], with_prior_preservation=args.with_prior_preservation)

            if vae is not None:
                # Convert images to latent space
                model_input = vae.encode(batch["pixel_values"].to(device, dtype=weight_dtype)).latent_dist.sample()
            ...
            # print(
            #     f"Step #{step}, loss: {loss.detach().item()}, prior_loss: {prior_loss.detach().item()}, instance_loss: {loss.detach().item()}"
            # )

    return [unet, text_encoder], custom_diffusion_layers
...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        if True:
            ...
            if vae is not None:
                # Convert images to latent space
                model_input = vae.encode(perturbed_images.to(device, dtype=weight_dtype)).latent_dist.sample()
                model_input = model_input * vae.config.scaling_factor
            else:
            ...
            noise_out_layers_features_3 = save_feature_maps(unet.up_blocks, unet.down_blocks, unet.mid_block)
            ...
            with torch.no_grad():
                if vae is not None:
                    # Convert images to latent space
                    clean_input = vae.encode(data_tensor.to(device, dtype=weight_dtype)).latent_dist.sample()
                    clean_input = clean_input * vae.config.scaling_factor
                else:
                    clean_input = perturbed_images
                # Sample noise that we'll add to the model input
                if args.offset_noise:
                    noise = torch.randn_like(clean_input) + 0.1 * torch.randn(
                        clean_input.shape[0], clean_input.shape[1], 1, 1, device=clean_input.device
                    )
                else:
                    noise = torch.randn_like(clean_input)
                bsz, channels, height, width = clean_input.shape

                clean_noisy_model_input = noise_scheduler.add_noise(clean_input, noise, timesteps)

                # Get the text embedding for conditioning
                clean_encoder_hidden_states = encode_prompt(
                    text_encoder,
                    input_ids,
                    None,
                    text_encoder_use_attention_mask=args.text_encoder_use_attention_mask,
                ...
                # Predict the noise residual
                clean_pred = unet(
                    clean_noisy_model_input, timesteps, clean_encoder_hidden_states, class_labels=class_labels, return_dict=False
                )[0]
                clean_out_layers_features_3 = save_feature_maps(unet.up_blocks, unet.down_blocks, unet.mid_block)
            ...
            if args.snr_gamma is None:
                ...
                loss = loss.mean(dim=list(range(1, len(loss.shape)))) * mse_loss_weights
                loss = loss.mean()
            feature_loss = F.mse_loss(noise_out_layers_features_3.float(), clean_out_layers_features_3.float(),
                                     reduction="mean")
            loss += feature_loss
        ...
        for step in range(args.search_steps):
            ...
            if len(res_time_seq) > 100:
                ...
                for index in range(0, 5):
                    ...
                    if vae is not None:
                        # Convert images to latent space
                        model_input = vae.encode(id_image.to(device, dtype=weight_dtype)).latent_dist.sample()
                        model_input = model_input * vae.config.scaling_factor
                    else:
                        model_input = id_image
                    # Sample noise that we'll add to the model input
                    if args.offset_noise:
                        noise = torch.randn_like(model_input) + 0.1 * torch.randn(
                            model_input.shape[0], model_input.shape[1], 1, 1, device=model_input.device
                ...
                if del_t < args.search_delete:
                    del_t = args.search_delete
                elif del_t > (1000 - args.search_delete):
                    del_t = 1000 - args.search_delete
...
def set_unet_attr(unet):
    ...
    for cross_module in cross_atttn_module_list:
        cross_module.forward = attn_forward(cross_module)
        setattr(cross_module, 'in_layers_features', None)
        setattr(cross_module, 'out_layers_features', None)


def save_feature_maps(up_blocks, down_blocks, mid_block):
    mid_layers_features_list = []

    out_layers_features_list_1 = []
    out_layers_features_list_2 = []
    out_layers_features_list_3 = []
    out_attn_q_features_list_1 = []
    out_attn_q_features_list_2 = []
    out_attn_q_features_list_3 = []
    ...
    for block in up_blocks:
        feature_type = "up_blocks"
        if block_idx == 3:
            for index in up_attn_list:
                out_layers_features_list_3.append(
                    block.attentions[index].transformer_blocks[0].attn2.out_layers_features)
...
def main(args):
    ...
    def load_image(img_dir, size=512, center_crop=True):
        ...
        images = [Image.open(i).convert("RGB") for i in list(Path(img_dir).iterdir())]
        images = [image_transforms(i) for i in images]
        images = torch.stack(images).contiguous()
        return images
    clean_data = load_image(
        args.instance_data_dir_for_train,
        size=args.resolution,
        center_crop=args.center_crop,
    )
    perturbed_data = load_image(
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("simac", config=tracker_config)

    logger.info("***** Select timestep *****")
    ...
    logger.info(f"  DreamBooth train steps = {args.max_f_train_steps}")
    logger.info(f"  PGD steps = {args.max_adv_train_steps}")
    logger.info(f"  PGD alpha = {args.pgd_alpha}")
    logger.info(f"  PGD eps = {args.pgd_eps}")
    ...
    for i in range(args.max_train_steps):
        ...
        custom_diffusion_layers_sur = copy.deepcopy(custom_diffusion_layers)

        f_sur, custom_diffusion_layers_sur = train_dreambooth(
            args,
            f_sur,
            custom_diffusion_layers_sur,
            tokenizer,
            noise_scheduler,
            vae,
            clean_data,
        ...
        f, custom_diffusion_layers = train_dreambooth(
            args,
            f,
            custom_diffusion_layers,
            tokenizer,
