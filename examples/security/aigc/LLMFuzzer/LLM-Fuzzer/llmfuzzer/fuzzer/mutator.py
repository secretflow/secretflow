...
class LLMMutatorGenerateSimilar(LLMMutatorBase):
    def __init__(
        ...
        super().__init__(
            model, temperature, max_tokens, max_trials, failure_sleep_time, fuzzer
        )
    ...
    def mutate_single(self, seed):
        return super().mutate_single(
            self.generate_similar(seed, self.fuzzer.prompt_nodes)
        )
...
class LLMMutatorExpand(LLMMutatorBase):
    ...
    def mutate_single(self, seed):
        return [
            r + seed
            for r in super().mutate_single(self.expand(seed, self.fuzzer.prompt_nodes))
        ]
...
class MutatePolicy:
    def __init__(self, mutators: 'list[Mutator]', fuzzer=None):
...
class MutateRandomSinglePolicy(MutatePolicy):
    def __init__(
        self, mutators: 'list[Mutator]', fuzzer=None, concatentate: bool = True
    ):
    ...
    def mutate_single(self, prompt_node):
        ...
        return [
            PromptNode(self.fuzzer, result, parent=prompt_node, mutator=mutator)
