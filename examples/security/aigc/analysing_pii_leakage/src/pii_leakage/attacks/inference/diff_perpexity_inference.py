...
class DiffPerplexityInferenceAttack(ReconstructionAttack):
    """
    ...
    def attack(self, baseline_lm: LanguageModel, lm: LanguageModel, target_sequence: str = None, pii_candidates: List[str] = None, *args, **kwargs):
        """
        ...
        ppls_baseline = baseline_lm.perplexity(queries, return_as_list=True, verbose=kwargs.setdefault('verbose', True))
        # visualize the perplexity and the baseline perplexity and the diff using matplotlib
        diffs = [ppl/ppl_baseline for ppl, ppl_baseline in zip(ppls, ppls_baseline)]
        ...
        return ppls, ppls_baseline, results, candidates
