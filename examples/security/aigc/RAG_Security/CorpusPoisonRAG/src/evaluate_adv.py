...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="contriever",
        choices=["contriever-msmarco", "contriever", "dpr-single", "dpr-multi", "ance"],
    )
    parser.add_argument(
        "--attack_dataset",
        type=str,
        default="nq-train",
        choices=["nq-train", "msmarco", "nq"],
    )
    parser.add_argument(
        "--advp_path",
        type=str,
        default="results/advp",
        help="the path where generated adversarial passages are stored",
    )
    parser.add_argument(
        "--num_advp",
        type=str,
        default="50",
        help="how many adversarial passages are generated (i.e., k in k-means); you may test multiple by passing `--num_advp 1,10,50`",
    )
    ...
    parser.add_argument(
        '--eval_dataset', type=str, default="fiqa", help='BEIR dataset to evaluate'
    )
    ...
    if args.orig_beir_results is None:
        print(
            "Please evaluate on BEIR first -- %s on %s"
            % (args.eval_model_code, args.eval_dataset)
        )
    ...
    def evaluate_adv(prefix, k, qrels, results):
        ...
        adv_input = {
            'input_ids': adv_p_ids,
            'attention_mask': adv_attention,
            'token_type_ids': adv_token_type,
        }
