...
class DeepCrossbase(nn.Module):
    def __init__(self, config, dense_features_cols, sparse_features_cols):
        ...
        self.embedding_layers = nn.ModuleList([
            nn.Embedding(num_embeddings=e[0], embedding_dim=e[1], scale_grad_by_freq=True) 
...
class DeepCrossfuse(nn.Module):
    def __init__(self, config, dense_features_cols_alice,dense_features_cols_bob, sparse_features_cols_alice,sparse_features_cols_bob):
        super(DeepCrossfuse, self).__init__()
        self._config = config

        self.dnn_input_dim = self._config['dnn_input_dim']
        # 稠密和稀疏特征的数量
        self._num_of_dense_feature = len(dense_features_cols_alice)+len(dense_features_cols_bob)
        

        # 对于稀疏特征，会根据其类别数来计算嵌入维度（通常公式是 6 * 类别数的1/4次方），可以有效减少类别特征的维度，同时保持足够的信息表达能力。
        self.embedding_dim_alice = list(map(lambda x: int(6 * pow(x, 0.25)), sparse_features_cols_alice))   
        self.embedding_dim_bob = list(map(lambda x: int(6 * pow(x, 0.25)), sparse_features_cols_bob))   
        
        self.cross_out_num = self._num_of_dense_feature + sum( self.embedding_dim_alice)  + sum(self.embedding_dim_bob) 
        # 初始化
    ...
    def forward(self, deep_outs, cross_outs, xTws):
        ...
        xTws_0 = xTws_0.expand(-1, cross_out_1.shape[1])  # 将 xTws_0 的第二维广播到 D 维
        xTws_1 = xTws_1.expand(-1, cross_out_0.shape[1])  # 将 xTws_1 的第二维广播到 D 维
