...
def sf_train(clients, server, epochs, train_dataset,batch_size):
    ...
    for epoch in range(epochs):
        ...
        for batch_id, (alice_data, bob_data, labels) in enumerate(train_loader):
            ...
            total+=loss.item()
            logging.warning('[Training Epoch: {}] Batch: {}, Loss: {}'.format(epoch, batch_id, loss))
            # global_weights = server.get_weights()  # 从服务器获取全局权重
            setting = []
            for client in clients:
                ret = client.update_gradients(loss)  # Client 端根据 server 的 loss 更新梯度
                setting.append(ret)
            ########################
            # 等待一批训练完成
            ########################
...
if __name__ == '__main__':
    ...
    columns_for_alice = ['I1', 'I3', 'I5', 'I7', 'I9', 'I11', 'I13',  
                        'C1', 'C3', 'C5', 'C7', 'C9', 'C11', 'C13', 
                        'C15', 'C17', 'C19', 'C21', 'C23', 'C25']


    columns_for_bob = ['I2', 'I4', 'I6', 'I8', 'I10', 'I12',  
                        'C2', 'C4', 'C6', 'C8', 'C10', 'C12', 'C14', 
                        'C16', 'C18', 'C20', 'C22', 'C24', 'C26','Label']
    ...
    df_data_alice,df_data_bob,labels, dense_features_cols_alice, sparse_features_cols_alice,dense_features_cols_bob, sparse_features_cols_bob = getTrainData(deepcrossbase_config['train_file'], deepcrossbase_config['fea_file'],columns_for_alice ,columns_for_bob )

    client_model_alice = DeepCrossbase(deepcrossbase_config, dense_features_cols=dense_features_cols_alice, sparse_features_cols=sparse_features_cols_alice)
    client_model_bob = DeepCrossbase(deepcrossbase_config, dense_features_cols=dense_features_cols_bob, sparse_features_cols=sparse_features_cols_bob)
    server_model=DeepCrossfuse(deepcrossfuse_config,dense_features_cols_alice,dense_features_cols_bob, sparse_features_cols_alice,sparse_features_cols_bob)
    train_dataset = Data.TensorDataset(torch.tensor(df_data_alice).float(),torch.tensor(df_data_bob).float(), torch.tensor(labels).float())

    clients = [Client(client_model_alice,deepcrossbase_config,0,2, device=alice_pyu), Client(client_model_bob ,deepcrossbase_config,1,2, device=bob_pyu)]
