...
def val(model, local_rank, cfg):
    ...
    dataloader = load_data(cfg, mode='val', model=model, local_rank=local_rank)  # 加载验证数据
    tasks = []
...
def main_worker(local_rank, cfg):
    ...
    dist.init_process_group(backend='nccl', #NVIDIA提供的高效分布式通信库，为GPU优化
                            init_method='tcp://127.0.0.1:23456', #所有的进程通过该地址进行通信
                            world_size=cfg.gpu_num,  # 总GPU数量
                            rank=local_rank)  # 当前进程的GPU编号
