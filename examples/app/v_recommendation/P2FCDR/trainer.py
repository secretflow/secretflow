...
class ModelTrainer(Trainer):
    def __init__(self, args, num_users, num_items):
        ...
        self.optimizer = train_utils.get_optimizer(args.optimizer, self.params, args.lr)
    ...
    def train_batch(
        ...
        interactions = [torch.LongTensor(x).to(self.device) for x in interactions]
        ...
        mlp_vector, mf_vector, neg_mlp_vector, neg_mf_vector = self.model(
            users, items, neg_items, U_mlp_g=self.U_mlp_g[0], U_mf_g=self.U_mf_g[0]
        )
        loss = self.p2fcdr_loss_fn(mlp_vector, mf_vector, neg_mlp_vector, neg_mf_vector)
        if "Fed" in self.method and args.mu:
            loss += self.prox_reg(
                [dict(self.model.encoder.named_parameters())], global_params, args.mu
            )
    ...
    def p2fcdr_loss_fn(self, mlp_vector, mf_vector, neg_mlp_vector, neg_mf_vector):
    ...
    def prox_reg(self, params1, params2, mu):
        ...
        for branch_params1, branch_params2 in zip(params1, params2):
            branch_params2 = [branch_params2[key] for key in branch_params1.keys()]
