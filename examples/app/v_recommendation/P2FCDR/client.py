...
class Client:
    def __init__(
        self, model_fn, c_id, args, train_dataset, valid_dataset, test_dataset
    ):
    ...
    def load_dataset(self):
        ...
        self.model_id = (
            self.args.model_id
            if len(self.args.model_id) > 1
            else "0" + self.args.model_id
        )
        ...
        for _ in range(args.local_epoch):
            ...
            for user_ids, interactions in self.train_dataloader:
                batch_loss = self.trainer.train_batch(
                    user_ids, interactions, round, args, global_params=global_params
                )
                loss += batch_loss
                step += 1
            gc.collect()
        logging.warning(
            "Epoch {}/{} - client {} -  Training Loss: {:.3f}".format(
    ...
    def evaluation_logging(self, eval_logs, mode="valid"):
        ...
        logging.warning(
            "NDCG @5|10: %.4f \t %.4f" % (eval_log["NDCG @5"], eval_log["NDCG @10"])
        )
    ...
    def save_params(self):
        ...
        ckpt_filename = os.path.join(method_ckpt_path, "client%d.pt" % self.c_id)
