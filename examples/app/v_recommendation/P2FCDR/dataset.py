...
class RecDataset(Dataset):
    ...
    def __init__(self, args, domain, model="GNN", mode="train", load_prep=True):
        ...
        self.dataset_dir = os.path.join(
            self.data_dir,
            self.domain + "_" + "".join([domain[0] for domain in self.args.domains]),
        )
        (
            self.user_ids,
            self.interactions,
    ...
    def read_data(self, dataset_dir):
        with open(
            os.path.join(dataset_dir, "num_users.txt"), "rt", encoding="utf-8"
        ) as infile:
            num_users = int(infile.readline())
        with open(
            os.path.join(dataset_dir, "num_items.txt"), "rt", encoding="utf-8"
        ) as infile:
            num_items = int(infile.readline())

        with open(
            os.path.join(self.dataset_dir, "%s_data.txt" % self.mode),
            "rt",
            encoding="utf-8",
        ) as infile:
            user_ids, items = [], []
    ...
    def preprocess(self, interactions, dataset_dir, load_prep):
        ...
        self.prep_data_path = os.path.join(
            dataset_dir, self.prep_dir, "%s_%s_data.pkl" % (self.model, self.mode)
        )
        if os.path.exists(self.prep_data_path) and load_prep:
            ...
            print(
                "Successfully load preprocessed %s %s data!" % (self.domain, self.mode)
            )
        else:
            prep_interactions = prep_functions[self.model](interactions, mode=self.mode)
            with open(self.prep_data_path, "wb") as infile:
                pickle.dump(prep_interactions, infile)
            print("Successfully preprocess %s %s data!" % (self.domain, self.mode))
