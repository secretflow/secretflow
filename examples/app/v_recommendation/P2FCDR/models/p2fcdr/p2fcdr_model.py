...
class P2FCDR(nn.Module):
    def __init__(self, num_users, num_items, args):
        ...
        # User embeddings
        self.user_mlp_emb = nn.Embedding(num_users, config.emb_size)
        self.user_mf_emb = nn.Embedding(num_users, config.emb_size)
        # Item embeddings cannot be shared between clients, because the numbers
        # of items in each domain are different
        self.item_mlp_emb = nn.Embedding(num_items, config.emb_size)
        self.item_mf_emb = nn.Embedding(num_items, config.emb_size)
    ...
    def get_user_embeddings(self):
        self.user_index = torch.arange(0, self.user_mlp_emb.num_embeddings, 1).to(
            self.device
        )
        self.item_index = torch.arange(0, self.item_mlp_emb.num_embeddings, 1).to(
            self.device
        )
        # user_mlp_emb, user_mf_emb: (num_users, emb_size)
        self.U_mlp = self.my_index_select_embedding(self.user_mlp_emb, self.user_index)
        self.U_mf = self.my_index_select_embedding(self.user_mf_emb, self.user_index)
        self.V_mlp = self.my_index_select_embedding(self.item_mlp_emb, self.item_index)
        self.V_mf = self.my_index_select_embedding(self.item_mf_emb, self.item_index)
