...
def sf_train(clients, server, epochs, train_dataset_params, batch_size):
    ...
    for epoch in range(epochs):
        ...
        for batch_id, (alice_batch, bob_batch) in enumerate(zip(alice_loader, bob_loader)):
            # 获取客户端outputs
            client_outputs = []
            for client_id, (client, batch) in enumerate(zip(clients, [bob_batch, alice_batch])):
            ...
            if batch_id % 10 == 0:
                logging.warning(f'[Training Epoch: {epoch}] Batch: {batch_id}, Loss: {loss}')
...
if __name__ == '__main__':
    ...
    clients = [
        ...
        Client(bahe_model, bert4rec_encoder, args, 1, 2, device=alice_pyu)  # Alice处理域2
    ]
    ...
    # 开始训练
    sf_train(clients, server, epochs=50, train_dataset_params=train_dataset_params, batch_size=32)