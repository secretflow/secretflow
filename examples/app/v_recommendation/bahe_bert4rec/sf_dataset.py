...
class DualDomainSeqDataset(data.Dataset):
    def __init__(self, seq_len, isTrain, neg_nums, long_length, pad_id, csv_path, domain_id):
    ...
    def __getitem__(self, idx):
        ...
        if domain_id_old == 0:
            ...
            if self.isTrain:
                neg_samples = random.sample(list(neg_items_set), 1) if neg_items_set else [0]
                label.append(0)
            else:
                neg_samples = random.sample(list(neg_items_set), min(self.neg_nums, len(neg_items_set)))
                neg_samples += [0] * (self.neg_nums - len(neg_samples))  # 补齐不足的部分
        ...
        # 生成行为文本
        behavior_texts = self.__generate_behavior_texts__(seq_d1_raw, seq_d2_raw, domain_id)
...
def collate_fn_enhance(batch):
    ...
    label = torch.stack([torch.Tensor(sample['label']) for sample in batch], dim=0).squeeze(1)
    domain_id = torch.cat([torch.LongTensor(sample['domain_id']) for sample in batch], dim=0)
