...
class BERT4Rec(nn.Module):
    ...
    def __init__(self, user_length, user_emb_dim, item_length, item_emb_dim, seq_len, hid_dim, bs, isInC, isItC, threshold1, threshold2):
        super(BERT4Rec, self).__init__()
        self.user_embedding = EmbeddingLayer(user_length, user_emb_dim)
        self.item_embedding = EmbeddingLayer(item_length, item_emb_dim)
        self.position_embedding = nn.Embedding(seq_len + 1, item_emb_dim)  # +1 确保序列长度足够
    ...
    def process_sequence(self, item_ids, seq, domain_ids, user_embedding):
        ...
        # 调整维度
        domain_emb = domain_emb.unsqueeze(1).expand(-1, seq_len, -1)  # [batch_size, seq_len, embed_dim]
        pos_emb = pos_emb.unsqueeze(0).expand(batch_size, -1, -1)  # [batch_size, seq_len, embed_dim]
    ...
    def forward(self, user_embedding, item_ids, seq_d1, seq_d2, domain_ids):
        output_d1 = self.process_sequence(item_ids, seq_d1, domain_ids, user_embedding)  # [batch_size]
        output_d2 = self.process_sequence(item_ids, seq_d2, domain_ids, user_embedding)  # [batch_size]

        # 根据 domain_ids 选择对应的输出
        selected_output = torch.where(domain_ids == 0, output_d1, output_d2)  # [batch_size]
...
class AtomicBehaviorEmbedding(nn.Module):
    ...
    def forward(self, behavior_texts):
        ...
        for length in batch_lengths:
            if length > 0:
                user_embeddings = all_embeddings[start_idx:start_idx + length].mean(dim=0)
            else:
                # 对于没有行为的用户，使用零向量
...
class SimpleTextEncoder(nn.Module):
    def __init__(self, vocab_size=30000, embed_dim=128):  # 改为128维度
        ...
        self.encoder = nn.LSTM(embed_dim, embed_dim//2, 
...
class BAHE(nn.Module):
    ...
    """
    def __init__(self, albert_model_name, embed_dim, num_heads, ff_dim, num_layers, dropout=0.1):
        ...
        self.interaction_model = BehaviorInteractionModel(embed_dim, num_heads, ff_dim, num_layers, dropout)
        
    def forward(self, behavior_texts):
        # 获取文本嵌入
        behavior_embeddings = self.atomic_embedding(behavior_texts)
        # 添加序列维度并投影到正确的维度
        behavior_embeddings = behavior_embeddings.unsqueeze(1)  # [batch_size, 1, embed_dim]
...
class BERT4RecFusion(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, ff_dim):
        super().__init__()
        self.transformer = TransformerBlock(embed_dim, num_heads, ff_dim)  # 改名为transformer
        self.predict_layer = nn.Linear(embed_dim, 1)
        
    def forward(self, user_embedding_d1, user_embedding_d2, seq_embedding_d1, seq_embedding_d2, domain_ids):
        # 根据domain_id选择正确的表示
        selected_user_embedding = torch.where(
            domain_ids.unsqueeze(-1) == 0,
            user_embedding_d1,
            user_embedding_d2
        )
