...
if __name__ == "__main__":
    ...
    train_dataset = datasets.CIFAR100(
        "./dataset", train=True, download=True, transform=transform1
    )
    test_dataset = datasets.CIFAR100(
        "./dataset", train=False, download=True, transform=transform2
    )
    ...
    train_client_samples, test_client_samples = split_cifar100(
    ...
    for j in tqdm(range(len(clients))):
        ...
        model = NetworkCIFAR(
            args.init_channels, 100, args.layers, args.auxiliary, genotype
        )
