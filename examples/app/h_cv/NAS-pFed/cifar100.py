...
from dataSplit.cifar100.cifar100 import split_cifar100
...
def add_args(parser):
    ...
    parser.add_argument('--num_classes', type=int, default=100, help='数据集类别个数')
...
if __name__ == "__main__":
    ...
    train_client_samples, test_client_samples = split_cifar100(args.dirichlet, args.client_number)
    ...
    server = Server(train_loader, test_loader, 50000, args.client_number, device, args, device=server_pyu)

    clients = []
    for client_id in range(args.client_number):
        clients.append(Client(client_id + 1, search_dataloader[client_id], val_dataloader[client_id],
                              train_dataloader[client_id], test_dataloader[client_id], 50000 / args.client_number,
                              device, args, device=clients_id[client_id]))
    ...
    for j in tqdm(range(len(clients))):
        ...
        model = NetworkCIFAR(args.init_channels, 100, args.layers, args.auxiliary, genotype)
        ...
        for n in range(500):
            ...
            with open('./result/cifar100/client'+str(j)+'.txt', 'a') as file:
