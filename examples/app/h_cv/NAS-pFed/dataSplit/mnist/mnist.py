...
TEST_EXAMPLES = 10000
TRAIN_EXAMPLES_PER_LABEL = [5923, 6742, 5958, 6131, 5842, 5421, 5918, 6265, 5851, 5949]
TEST_EXAMPLES_PER_LABEL = [980, 1135, 1032, 1010, 982, 892, 958, 1028, 974, 1009]

current_dir = os.path.dirname(os.path.abspath(__file__))


def split_mnist(dirichlet_parameter: float = 0.1, total_clients: int = 500):
    # 加载 MNIST 数据集
    ...
    for k in range(total_clients):
        for i in range(train_examples_per_client):
            ...
            # print(str(len(train_indices_by_class)) +" "+str(len(train_indices_by_class[sampled_label]))+ " "+str(sampled_label)+" "+str(train_count[sampled_label]))
            # print(len(train_count), sampled_label)
            # print(sampled_label, len(train_indices_by_class[sampled_label]), train_count[sampled_label])
            train_client_samples[k].append(
                train_indices_by_class[sampled_label][train_count[sampled_label]]
            )
            train_count[sampled_label] += 1
            if train_count[sampled_label] == TRAIN_EXAMPLES_PER_LABEL[sampled_label]:
                # print("该类别的训练集已取完")
        ...
        for i in range(test_examples_per_client):
            ...
            if test_count[sampled_label] == TEST_EXAMPLES_PER_LABEL[sampled_label]:
                # print("该类别的测试集已取完")
