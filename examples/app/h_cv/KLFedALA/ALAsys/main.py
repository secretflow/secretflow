...
def train(clients, server, global_epochs, num_selected):
    loading = []
    distributions = []
    for c in clients:
        loading.append(c.load_data_and_ALA())
        # Collect the distribution of all clients
        dis = c.compute_class_distribution()
        distributions.append(dis.to(server.device))

    sf.wait(loading)
    sf.wait(distributions)
    selected_indices = server.select_clients(num_selected, distributions)
    selected_indices = sf.reveal(selected_indices)
...
def run(args):
    ...
    for i in range(args.number_of_times):
        ...
        if args.dataset == "mnist":
            ...
            args.model = cnn(3, num_classes=100, dim=1600).to(args.device)
        else:
            args.model = cnn(3, num_classes=200, dim=10816).to(args.device)
        ...
        train(clients, server, args.global_epochs, args.selected_clients)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-sc", "--selected_clients", type=int, default=20)
    ...
    parser.add_argument(
        "-nt", "--number_of_times", type=int, default=1, help="number of times"
    )
    parser.add_argument(
