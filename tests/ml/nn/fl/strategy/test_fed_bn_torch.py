...
from secretflow.ml.nn.fl.backend.torch.strategy.fed_bn import FedBN
from tests.ml.nn.fl.model_def import ConvNet


class TestFedBN:
    def test_fed_bn_local_step(self, sf_simulation_setup_devices):
        ...
        # Initialize FedBN strategy with ConvNet model
        conv_net_builder = ConvNetBuilder()
        fed_bn_worker = FedBN(builder_base=conv_net_builder)
        ...
        fed_bn_worker.train_set = iter(test_loader)
        fed_bn_worker.train_iter = iter(fed_bn_worker.train_set)

        # Perform a training step
        weights = None
        weights, num_sample = fed_bn_worker.train_step(
            weights, cur_steps=0, train_steps=1
        )

        # Apply weights update
        fed_bn_worker.apply_weights(weights)

        # Assert the sample number and length of weights
        assert num_sample == 32  # Batch size
        assert len(weights) == len(
            list(fed_bn_worker.model.parameters())
        )  # Number of model parameters

        # Perform another training step to test cumulative behavior
        _, num_sample = fed_bn_worker.train_step(weights, cur_steps=1, train_steps=2)
