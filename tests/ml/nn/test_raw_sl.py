...
from secretflow.ml.nn.sl_base import PYUModel
from tests.basecase import DeviceTestCase
from tests.ml.nn.model_def import SLConvNet, calculate_accu, get_mnist_dataloader
...
class TestRawSL(DeviceTestCase):
    def setUp(self) -> None:
        ...
        self.dl_fn_without_y = {
            'train': lambda: get_mnist_dataloader(
                is_train=True, use_cols=cols_1, with_y=False
            ),
            'test': lambda: get_mnist_dataloader(
                is_train=False, use_cols=cols_1, with_y=False
            ),
        }
        self.dl_fn_with_y = {
            'train': lambda: get_mnist_dataloader(is_train=True, use_cols=cols_2),
            'test': lambda: get_mnist_dataloader(is_train=False, use_cols=cols_2),
        }
        self.model_fn_without_y = lambda: SLConvNet(output_shape=32, fc_in_dim=72)
        self.model_fn_with_y = lambda: SLConvNet(
            output_shape=10, fc_in_dim=120, embed_shape_from_other=32, with_softmax=True
        )
    ...
    def test_simple_split_learning(self):
        ...
        model1 = PYUModel(
            device=self.alice,
            model_fn=self.model_fn_without_y,
            loss_fn=self.loss_fn,
            optim_fn=self.optim_fn,
            dataloader_fn=self.dl_fn_without_y,
        )
        # Model with y
        model2 = PYUModel(
            device=self.bob,
            model_fn=self.model_fn_with_y,
            loss_fn=self.loss_fn,
            optim_fn=self.optim_fn,
            dataloader_fn=self.dl_fn_with_y,
        )
        accu = 0
    ...
    def eval(model1, model2):
        ...
        y_pred_y = model2.forward(
            used_name='test', external_input={'embed_from_other': embed}
        )
