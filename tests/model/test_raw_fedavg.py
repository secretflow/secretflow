...
from secretflow.model.fl_base import PYUModel
from tests.basecase import DeviceTestCase
from tests.model.model_def import ConvNet, get_mnist_dataloader, calculate_accu
...
class TestRawFedAVG(DeviceTestCase):
    def setUp(self) -> None:
        ...
        self.dataloader_fn = {'train': lambda: get_mnist_dataloader(is_train=True),
                              'test': lambda: get_mnist_dataloader(is_train=False)}

    def test_simple_fedavg(self):
        model1 = PYUModel(device=self.alice, model_fn=self.model_fn, loss_fn=self.loss_fn, optim_fn=self.optim_fn,
                          dataloader_fn=self.dataloader_fn)
        model2 = PYUModel(device=self.bob, model_fn=self.model_fn, loss_fn=self.loss_fn, optim_fn=self.optim_fn,
                          dataloader_fn=self.dataloader_fn)
        ...
        for i in range(self.epochs):
            ...
            aggregate_fn = self.ppu(lambda grad1, grad2: [jnp.mean(jnp.array([g1, g2]), axis=0) for g1, g2 in zip(grad1,
                                                                                                                  grad2)])
            grad = aggregate_fn(grad1.to(self.ppu), grad2.to(self.ppu))
