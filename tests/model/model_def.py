...
from secretflow.model.fl_base import BaseModule
from secretflow.model.sl_base import SLBaseModule
...
class SLConvNet(SLBaseModule):
    ...
    def __init__(self, output_shape: int,
                 fc_in_dim: int, embed_shape_from_other: Optional[int] = None,
                 with_softmax=False):
...
def get_mnist_dataloader(is_train=True, loc='./data', num_batch_size=128, use_cols=None, with_y=True):
    with FileLock("./data.lock"):
        mnist = datasets.MNIST(
            loc, train=is_train, download=True,
