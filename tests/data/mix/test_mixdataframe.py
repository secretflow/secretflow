...
class TestHMixDataFrame(DeviceTestCase):
    ...
    def test_mean_should_ok(self):
        ...
        expected = pd.concat(
            [
                self.df_part0.mean(numeric_only=True),
                self.df_part1.mean(numeric_only=True),
            ]
        )
    ...
    def test_getitem_should_ok(self):
        ...
        pd.testing.assert_frame_equal(
            pd.concat(
                [
                    reveal(value_h.partitions[0].partitions[self.bob].data),
        ...
        pd.testing.assert_frame_equal(
            pd.concat(
                [
                    reveal(value_v.partitions[1].partitions[self.alice].data),
    ...
    def test_construct_should_error_when_diff_part_types(self):
        with self.assertRaisesRegex(
            AssertionError, 'All partitions should have same type'
        ):
            MixDataFrame([self.h_part0, self.v_part0])

    def test_construct_should_error_when_none_or_empty_parts(self):
        with self.assertRaisesRegex(
            AssertionError, 'Partitions should not be None or empty.'
        ):
            MixDataFrame()
