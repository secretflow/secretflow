...
class TestDevicePSI2PC(DeviceTestCase):
    def setUp(self) -> None:
        da = pd.DataFrame({'c1': ['K5', 'K1', 'K2', 'K6', 'K4', 'K3'],
                           'c2': ['A5', 'A1', 'A2', 'A6', 'A4', 'A3'],
                           'c3': [5, 1, 2, 6, 4, 3]})

        db = pd.DataFrame({'c1': ['K3', 'K1', 'K9', 'K4'],
                           'c2': ['B3', 'A1', 'A9', 'A4'],
                           'c3': [3, 1, 9, 4]})

        db2 = pd.DataFrame({'c1': ['K3', 'K1', 'K1', 'K4'],
                            'c2': ['B3', 'A1', 'A1', 'A4'],
                            'c3': ['C3', 'C1', 'D1', 'C4'],
                            'c4': [3, 1, 9, 4]})

        db3 = pd.DataFrame({'c1': ['K7', 'K8', 'K9'],
                            'c2': ['A7', 'A8', 'A9'],
                            'c3': [7, 8, 9]})
    ...
    def test_single_col(self):
        da, db = self.ppu.psi_df('c1', [self.da, self.db])

        expected = pd.DataFrame({'c1': ['K1', 'K3', 'K4'],
                                 'c2': ['A1', 'A3', 'A4'],
                                 'c3': [1, 3, 4]})
    ...
    def test_multiple_col(self):
        da, db = self.ppu.psi_df(['c1', 'c2'], [self.da, self.db])

        expected = pd.DataFrame({'c1': ['K1', 'K4'],
                                 'c2': ['A1', 'A4'],
                                 'c3': [1, 4]})
        pd.testing.assert_frame_equal(sf.reveal(da).reset_index(drop=True), expected)
        pd.testing.assert_frame_equal(sf.reveal(db).reset_index(drop=True), expected)
    ...
    def test_missing_col(self):
        with self.assertRaisesRegex(RuntimeError, "can't find feature names 'c4'"):
            da, db = self.ppu.psi_df(['c1', 'c4'], [self.da, self.db2])
            sf.reveal([da, db])
    ...
    def test_no_intersection(self):
        da, db = self.ppu.psi_df('c1', [self.da, self.db3])
    ...
    def test_psi_csv(self):
        ...
        self.ppu.psi_csv(['c1', 'c2'], input_path, output_path)
...
class TestDevicePSI3PC(DeviceTestCase):
    def setUp(self) -> None:
        ...
        dc = pd.DataFrame({'c1': ['K9', 'K4', 'K3', 'K1', 'k8'],
                           'c2': ['A9', 'B4', 'B3', 'A1', 'k8'],
                           'c3': [9, 4, 3, 1, 8]})
    ...
    def test_single_col(self):
        da, db, dc = self.ppu.psi_df('c1', [self.da, self.db, self.dc], protocol='ecdh')
    ...
    def test_multiple_col(self):
        da, db, dc = self.ppu.psi_df(['c1', 'c2'], [self.da, self.db, self.dc], protocol='ecdh')
        expected = pd.DataFrame({'c1': ['K1'],
                                 'c2': ['A1'],
                                 'c3': [1]})
        pd.testing.assert_frame_equal(sf.reveal(da).reset_index(drop=True), expected)
        pd.testing.assert_frame_equal(sf.reveal(db).reset_index(drop=True), expected)
