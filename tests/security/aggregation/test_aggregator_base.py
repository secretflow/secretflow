...
class TestAggregatorBase:
    ...
    def test_average_on_single_without_weights_should_ok(self):
        ...
        # WHEN
        sum = sf.reveal(self.aggregator.average([a, b], axis=0))

        # THEN
        np.testing.assert_almost_equal(
            sum, np.array([[6.0, 7.0, 8.0], [9.0, 10.0, 11.0]]), decimal=5
        )
    ...
    def test_average_on_list_without_weights_should_ok(self):
        ...
        # THEN
        np.testing.assert_almost_equal(
            sum[0], np.array([[6, 7, 8], [9, 10, 11]]), decimal=5
        )
        np.testing.assert_almost_equal(
            sum[1], np.array([[26, 27, 28], [29, 30, 31]]), decimal=5
        )
    ...
    def test_average_on_list_with_weights_should_ok(self):
        ...
        # THEN
        np.testing.assert_almost_equal(
            sum[0], np.array([[7, 8, 9], [10, 11, 12]]), decimal=4
        )
        np.testing.assert_almost_equal(
            sum[1], np.array([[27, 28, 29], [30, 31, 32]]), decimal=4
        )
