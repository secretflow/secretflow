...
def test_vtable_null(comp_prod_sf_cluster_config):
    ...
    df: CompVDataFrame = CompVDataFrame.load(
        storage, dd, col_selects=["id", "col2", "col1", "col3"]
    )
    # check columns order
    assert df.columns == ["id", "col2", "col1", "col3"]

    out_tbl = df.dump(storage, output_path)
    # read orc
    orc_tbl: CompVDataFrame = CompVDataFrame.load(
        storage, out_tbl, col_selects=["col3", "id", "col2", "col1"]
    )
    assert orc_tbl.shape == (4, 4)
    assert orc_tbl.columns == ["col3", "id", "col2", "col1"]
