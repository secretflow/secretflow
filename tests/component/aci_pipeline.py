...
if __name__ == "__main__":
    ...
    attrs = {
        ...
        "input/input_ds1/keys": ["id0"],
        "input/input_ds2/keys": ["id1"],
    }
    # 测试psi
    psi = TestComp("psi_test", "data_prep", "psi", "1.0.0", attrs)
    aci_pipe.add_comp(psi, ["DAGInput.alice", "DAGInput.bob"])

    attrs = {
        "input/input_ds/drop_features": ["alice1", "bob9"],
    }
    # 测试feature_filter
    ...
    # 测试ss_sgd_train
    sslr = TestComp("sslr_train", "ml.train", "ss_sgd_train", "1.0.0", attrs)
    ...
    # 测试ss_sgd_predict
    sslr = TestComp("sslr_pred", "ml.predict", "ss_sgd_predict", "1.0.0", attrs)
