...
from ..attack.model_def import CafeServer, LocalEmbedding
...
def do_test_sl_and_lia(alice, bob, carol):
    ...
    fake_grad = CAFEFakeGradients(attack_party=carol)
    ...
    history = sl_model.fit(
        ...
        shuffle=False,
        random_seed=1234,
        callbacks=[cafe_attack, fake_grad],
    )
    print(history)
...
def test_sl_and_lia(sf_simulation_setup_devices):
    ...
    do_test_sl_and_lia(devices.alice, devices.bob, devices.carol)
