...
def test_create_dataset_builder(sf_simulation_setup_devices):
    ...
    dataset_builder = data.create_dataset_builder(
        pyus=[devices.alice, devices.bob],
        label_pyu=devices.alice,
        label_name="y",
        batch_size=2,
        epochs=2,
    ...
    alice_builder = dataset_builder[devices.alice]
    bob_builder = dataset_builder[devices.bob]
...
def test_create_model_builder(sf_simulation_setup_devices):
    ...
    configs = compile.compile_by_initiator(
        parties=[devices.alice, devices.bob],
        initiator=devices.alice,
        models=MODELS_CODE,
        learning_rate=0.01,
        loss="",
...
def test_compile_by_self(sf_simulation_setup_devices):
    ...
    assert configs[devices.alice] is not None and configs[devices.bob] is not None
