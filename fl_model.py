...
class FLModel:
    ...
    def initialize_weights(self):
        ...
        for device, worker in self._workers.items():
            worker.on_epoch_begin(0)
            weights = worker.get_weights()
            clients_weights.append(weights)
            
        if self._aggregator is not None:
            initial_weight = self._aggregator.average(data=clients_weights, axis=0)
            for device, worker in self._workers.items():
                weights = (
        ...
        #初始化         
        self._aggregator.initial_scaffold(initial_weight)
        for device, worker in self._workers.items():
            worker.initial_scaffold()
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, train_steps_per_epoch, aggregate_freq):
                ...
                client_control_list=[]
                client_delta_y_list=[]
                client_delta_control_list=[]
                server_control=self._aggregator._control
                ...
                for idx, device in enumerate(self._workers.keys()):
                    ...
                    # client_params=self._aggregator.x_weight
                    # client_params=client_params.to(device)
                    
                    server_control=server_control.to(device)
                    
                    client_params, sample_num, control, delta_y, delta_control  = self._workers[device].train_step(
                        client_params,
                        server_control,
                    ...
                    client_control_list.append(control)
                    client_delta_y_list.append(delta_y)
                    client_delta_control_list.append(delta_control)
                    res.append(client_params)
                #####
                
                #更新变成client_delta_y_list            
                if self._aggregator is not None:
                    model_params = self._aggregator.average(
                        data=client_delta_y_list ,axis=0, weights=sample_num_list
                    )
                #更新服务器端的变量                     
                    self._aggregator.update_scaffold(client_delta_control_list,axis=0, weights=sample_num_list)
                else:
