...
class HESSLogisticRegression:
    ...
    def fit(
        ...
        for epoch in range(epochs):
            for step in range(steps_per_epoch):
                logging.info(f"epoch {epoch}/{epochs}, step: {step}/{steps_per_epoch} begin")
    ...
    def save_hess_sgd_model(self, ctx, feature_selects, label_col, dist_data_uri: str) -> dict:
        """
        Save fit model in LinearModel format.
        """
        public_info = {
            "reg_type": RegType.Logistic.value,
            "sig_type": SigType.T1.value,
        ...
        model_db = model_dumps(
            ctx,
            str(DistDataType.HESS_SGD_MODEL),
            DistDataType.HESS_SGD_MODEL,
            MODEL_MAX_MAJOR_VERSION,
            MODEL_MAX_MINOR_VERSION,
            [self._w],
            json.dumps(public_info),
            dist_data_uri,
            None,
        )
        return {"output_model": model_db}

    def load_hess_sgd_model(self, ctx, party_a_id: str, party_b_id: str, feature_selects=None, label_col=None) -> None:
        """
        Load LinearModel format model.
        """
        ...
        model_info = {
            'major_version': MODEL_MAX_MAJOR_VERSION, 
            'minor_version': MODEL_MAX_MINOR_VERSION, 
            'public_info': json.dumps(public_info)
        }
        uris = [f'{party_a_id}/0', f'{party_b_id}/0']
        objs_party = [party_a_id, party_b_id]
        objs_uri = []
        ...
        saved_objs = [saved_obj]
        model_meta = DeviceObjectCollection(
            objs=saved_objs,
            public_info=json.dumps(model_info),
        )
        dist_data = DistData(
            name = str(DistDataType.HESS_SGD_MODEL),
            type = str(DistDataType.HESS_SGD_MODEL),
            system_info = None,
            data_refs=[
        ...
        model_objs, model_info = model_loads(
            ctx,
            dist_data,
            MODEL_MAX_MAJOR_VERSION,
            MODEL_MAX_MINOR_VERSION,
        ...
        ), f"model_objs {model_objs}, model_info {model_info}"

        model_info = json.loads(model_info)
        assert (
            isinstance(model_info, dict)
            and "reg_type" in model_info
            and "sig_type" in model_info
        )
        model = LinearModel(
            weights=model_objs[0],
            reg_type=RegType(model_info["reg_type"]),
            sig_type=SigType(model_info["sig_type"]),
        )
