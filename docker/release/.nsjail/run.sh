...
nsjail --config .nsjail/nsjail.cfg --chroot /root/sandbox/rootfs -Mo --rlimit_fsize max --hostname APP \
    ...
    --rlimit_as max --rlimit_nproc max --pass_fd 256 --keep_cap --proc_path /proc \
    -- /usr/local/bin/python -m secretflow.kuscia.entry --task_config_path /etc/kuscia/task-config.conf
